...
function renderConnectProviderGuides(guides: ReturnType<typeof listConnectProviderGuides>) {
  return [
    ...
    "Use `mailctl connect providers <provider>` for detailed commands and env requirements.",
    "API discovery: GET /api/connect and GET /api/connect/providers",
    ...guides.map(
      (guide) =>
        `${guide.id} | ${guide.displayName} | ${formatConnectSetupKind(guide.setupKind)} | inbound ${guide.inboundModes.join(", ")} | outbound ${guide.outboundModes.join(", ")}`
    )
  ].join("\n");
}
...
function renderConnectProviderGuide(guide: ReturnType<typeof resolveConnectProviderGuide> extends infer T
  ...
  return [
    ...
    ...(guide.authApi
      ...
      : []),
    `Recommended: ${guide.recommendedCommand}`,
    ...(guide.commands.length > 0 ? ["Commands:", ...guide.commands.map((command) => `  ${command}`)] : []),
...
function renderConnectOnboardingPlan(plan: ReturnType<typeof buildConnectOnboardingPlan>) {
  return [
    ...
    `Suggested account ID: ${plan.input.accountIdSuggestion}`,
    `Start here: ${plan.commands.login}`,
    `Provider guide: ${plan.commands.inspectProvider}`,
    `After login: ${plan.commands.observeAccounts}`,
    `Inspect rooms/internal mail: ${plan.commands.observeWorkbench}`,
    `Mailbox-centric view: ${plan.commands.observeInboxes}`,
    `Browser console: ${plan.console.browserPath}`,
    "OpenClaw migration path:",
    `  - start bridge mode: ${plan.migration.openClawUsers.startCommand}`,
    `  - inspect runtime boundary: ${plan.migration.openClawUsers.inspectRuntime}`,
    `  - inspect workbench projection: ${plan.migration.openClawUsers.inspectWorkbench}`,
    ...(plan.alternatives.length > 0
      ...
      : []),
    "Checklist:",
    ...plan.checklist.map((step) => `  - ${step}`),
    ...(plan.migration.openClawUsers.notes.length > 0
      ? ["OpenClaw notes:", ...plan.migration.openClawUsers.notes.map((note) => `  - ${note}`)]
      : []),
...
function createRuntimeFromEnv(options?: {
  gmailOAuthClient?: GmailOAuthClientLike;
  microsoftOAuthClient?: MicrosoftOAuthClientLike;
}) {
  ...
  process.once("exit", () => wrappedRuntime.close());
